...
import type { Dependencies, Options, AvailableTypes } from './types'
...
const without = (source: Dependencies, destination: Dependencies): Dependencies => source.filter(element => !destination.includes(element));

const intersection = (source: Dependencies, destination: Dependencies): Dependencies => source.filter(element => destination.includes(element));
...
const typesSync = async (options?: Options) => {
  ...
  const installedTypes = devDependencies.filter(
    d => d.indexOf("@types/") === 0
  );

  const availableTypes = await getTypes();

  const depsTypes = without(installedTypes.map(t => t.slice(7)), ignored);

  const typesToUninstall: Dependencies = without(depsTypes, dependenciesAfterExclude).map(t => `@types/${t}`);

  const typesAvailable: Dependencies = intersection(dependenciesAfterExclude, availableTypes.types).map(t => `@types/${t}`);
